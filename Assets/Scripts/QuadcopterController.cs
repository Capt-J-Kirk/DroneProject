...
public class QuadcopterController: MonoBehaviour
{
    ...
    private float Iyy = 1;
    private float Izz = 1;
    private float xCenterOfMass = 0;
    private float yCenterOfMass = 0;
    private float zCenterOfMass = 0;
    ...
    void Start()
    {
        ...
        // Set the center of mass, if it's not at the geometric center
        rb.centerOfMass = new Vector3(xCenterOfMass, yCenterOfMass, zCenterOfMass);

        // Define the inertia tensor
        Vector3 inertiaTensor = new Vector3(Ixx, Iyy, Izz);

        // Set the inertia tensor
        rb.inertiaTensor = inertiaTensor;

        // If needed, set the rotation of the inertia tensor relative to the Rigidbody
        rb.inertiaTensorRotation = Quaternion.identity; // Replace with actual rotation if not aligned
